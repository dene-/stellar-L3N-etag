...
export function hexToBytes(hex: string): Uint8Array {
  ...
  const bytes: number[] = [];
  for (let c = 0; c < clean.length; c += 2) bytes.push(parseInt(clean.slice(c, c + 2), 16));
  return new Uint8Array(bytes);
}

export function bytesToHex(data: ArrayBuffer | ArrayLike<number> | ArrayBufferLike): string {
  return new Uint8Array(data as ArrayBuffer).reduce((memo, i) => memo + ('0' + i.toString(16)).slice(-2), '');
}
...
export function canvas2bytes(canvas: HTMLCanvasElement, type: 'bw' | 'bwr' = 'bw'): number[] {
  ...
  for (let x = width - 1; x >= 0; x--) {
    for (let y = 0; y < height; y++) {
      ...
      if (type !== 'bwr') {
        buffer.push(
          imageData.data[index] > 0 && imageData.data[index + 1] > 0 && imageData.data[index + 2] > 0 ? 1 : 0
        );
      } else {
        buffer.push(
          imageData.data[index] > 0 && imageData.data[index + 1] === 0 && imageData.data[index + 2] === 0 ? 1 : 0
...
export async function ditheringCanvasByPalette(
  ...
  if ((output instanceof Uint8Array || output instanceof Uint8ClampedArray) && output.length === w * h * 4) {
